...
try {
    ...
    $stmt = $conexion->prepare(
        "INSERT INTO contacto (nombre, apellido, correo, telefono, asunto, mensaje)
         VALUES (?, ?, ?, ?, ?, ?)"
    );
    ...
    respuestaJson(true, 'Mensaje de contacto registrado exitosamente.');
} catch (Throwable $e) {
    error_log('Error guardar contacto: ' . $e->getMessage());
    respuestaJson(false, 'Error al guardar el mensaje de contacto.', null, 500);
}
