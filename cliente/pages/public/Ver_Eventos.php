    ...
    <nav class="mt-5">
        <ul class="pagination justify-content-center pagination-custom">
            ...
            for ($i = $inicio; $i <= $fin; $i++): ?>
                <li class="page-item <?= $i === $paginaActual ? 'active' : '' ?>"><a class="page-link" href="?pagina=<?= $i ?>&buscar=<?= urlencode($filtroBusqueda) ?>&estado=<?= urlencode($filtroEstado) ?>&fecha=<?= urlencode($filtroFecha) ?>"><?= $i ?></a></li>
            <?php endfor;
            if ($fin < $totalPaginas) {
                if ($fin < $totalPaginas - 1) echo '<li class="page-item disabled"><span class="page-link">…</span></li>';
                echo '<li class="page-item"><a class="page-link" href="?pagina=' . $totalPaginas . '&buscar=' . urlencode($filtroBusqueda) . '&estado=' . urlencode($filtroEstado) . '&fecha=' . urlencode($filtroFecha) . '">' . $totalPaginas . '</a></li>';
            }
            ?>
